...
def eta_sampling_with_temperature(logits, epsilon=6e-4, temperature=1.0, min_tokens_to_keep=1, return_logits=False):
...
def test_eta_sampling():
    ...
    sample_token, eta_logits = eta_sampling_with_temperature(logits, epsilon=0.1, temperature=1.0, min_tokens_to_keep=1, return_logits=True)
    print(eta_logits)
